...
public class DfuManager {
	...
	public void connect(Context context, BluetoothDevice device) {
		Logger.d(TAG, "Connecting device");
		mDevice = device;
	...
	private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
		...
		public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
			if (newState == BluetoothProfile.STATE_CONNECTED) {
				...
				Logger.i(TAG, "Device disconnected");
				...
				try {
					...
					if (refresh != null) {
						...
						Logger.d(TAG, "Device cache refresh result: " + success);
					}
		...
		public void onServicesDiscovered(BluetoothGatt gatt, int status) {
			...
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				for (BluetoothGattService service : services) {
					Logger.d(TAG, "Found Service: " + service.getUuid());
					if (service.getUuid().equals(DFU_SERVICE_UUID)) {
						Logger.d(TAG, "DFU Service found!");
		...
		public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			...
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				if (characteristic.getUuid().equals(DFU_CONTROLPOINT_CHARACTERISTIC_UUID) && !isFileSizeWritten) {
					Logger.d(TAG, "successfully written startDFU and now writing file size");
					writeFileSize();
					...
					Logger.d(TAG, "successfully written Packet received notification and now writing receive firmware image");
					...
					Logger.d(TAG, "successfully written ReceiveFirmwareImage and now writing file");
					startUploadingFile();
					isReceiveFirmwareImageWritten = true;
				}
			} else {
				Logger.e(TAG, ERROR_WRITE_CHARACTERISTIC + " [" + characteristic.getUuid() + "] Error code: " + status);
		...
		public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
			...
			if (opCode == RECEIVED_OPCODE && request == START_DFU) {
				Logger.d(TAG, "Received notification for StartDFU");
				if (characteristic.getValue()[2] == OPERATION_SUCCESS) {
					Logger.d(TAG, "File length is valid: " + characteristic.getValue()[2]);
					enablePacketNotification();
				} else {
					errorStatus = characteristic.getValue()[2];
					Logger.e(TAG, ERROR_FILE_LENGTH + " (" + errorStatus + ")");
			...
			//here we transfer next packet of 20 bytes
				...
				Logger.d(TAG, "Received Notification for sent Packet");
				...
				Logger.d(TAG, "Bytes received in Packet: " + receivedBytes);
				mCallbacks.onFileTranfering(receivedBytes);
				if (!isLastPacket && !mStopSendingPacket) {
					sendPacket();
				} else {
					Logger.d(TAG, "last packet notification received");
				}
			}
			...
			// if file has been transfered successfully the we will validate transfered file 
				...
				Logger.d(TAG, "File has been transfered");
				if (characteristic.getValue()[2] == OPERATION_SUCCESS) {
					Logger.d(TAG, "Successful File transfer!");
					mCallbacks.onFileTransferCompleted();
					validateFirmware();
				} else {
					errorStatus = characteristic.getValue()[2];
					Logger.d(TAG, ERROR_FILE_TRANSFER + errorStatus);
			...
			//if file has been validated successfully then we will activate and reset device 
				...
				Logger.d(TAG, "Transfered file has been validated");
				if (characteristic.getValue()[2] == OPERATION_SUCCESS) {
					Logger.d(TAG, "Successful File Transfer Validation!");
					mCallbacks.onFileTransferValidation();
					activateAndReset();
					isNotificationEnable = false;
				} else {
					errorStatus = characteristic.getValue()[2];
					Logger.e(TAG, ERROR_FILE_VALIDATION + errorStatus);
		...
		public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
			if (status == BluetoothGatt.GATT_SUCCESS) {
				if (isNotificationEnable) {
					...
					Logger.d(TAG, "Notification is disabled!");
				}
			} else {
	...
	public void enableNotification() {
		Logger.d(TAG, "Enable Notification");
	...
	public void disableNotification() {
		if (isNotificationEnable) {
			Logger.d(TAG, "Disable Notification");
	...
	private void writeFileSize() {
		...
		if (isDFUServiceFound) {
			...
			Logger.d(TAG, "writing File size");
	...
	private void enablePacketNotification() {
		Logger.d(TAG, "Enable Packet Notification");
	...
	private void receiveFirmwareImage() {
		Logger.d(TAG, "sending Receive Firmware Image message");
	...
	private void startUploadingFile() {
		Logger.d(TAG, "Preparing to send file");
		sendPacket();
	...
	private void validateFirmware() {
		...
		Logger.d(TAG, "writing validate Firmware value");
		mBluetoothGatt.writeCharacteristic(mDFUControlPointCharacteristic);
	}
	...
	private void activateAndReset() {
		...
		Logger.d(TAG, "writing activate and reset value");
		mBluetoothGatt.writeCharacteristic(mDFUControlPointCharacteristic);
	}
	...
	public void openFile(InputStream stream) {
		try {
			...
			Logger.d(TAG, "File Size: " + mFileSize);
		} catch (IOException e) {
			Logger.e(TAG, ERROR_FILE_OPEN + " " + e);
	...
	public void closeFile() {
		if (mFileStream != null) {
			try {
				...
				Logger.e(TAG, ERROR_FILE_CLOSE + " " + e.toString());
	...
	private void sendPacket() {
		...
		if (mPacketNumber == mTotalPackets) {
			Logger.d(TAG, "This is last packet, packet number: " + mPacketNumber);
			isLastPacket = true;
