...
public class HRSManager {
		...
		public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
			if (status == BluetoothGatt.GATT_SUCCESS) {
				if (characteristic.getUuid().equals(HR_SENSOR_LOCATION_CHARACTERISTIC_UUID)) {
					...
					Logger.d(TAG, "Body sensor location characteristic value: " + sensorPosition);
		...
		public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
			if (status == BluetoothGatt.GATT_SUCCESS) {
				...
				if (!isNotificationEnable) {
					...
					Logger.d(TAG, "Notification is set");
	...
	private void writeBatteryStateDescriptor() {
		if (isBatteryStateCharacteristicFound) {
			...
			Logger.e(TAG, "writing battery state desciptor");
			mBluetoothGatt.writeDescriptor(descriptor);
		}
	}
